...
| R2 | **GL processing-lane teardown SIGSEGV** at process exit | ROOT-CAUSED: `processing_lane`/`rendering_lane` are function-static singletons (`synthetic-stream-gl.cpp:112-116`) whose GPU objects free GL resources **in their destructors at process exit — after the GL context is gone**. The lanes provide `rs2_gl_shutdown_processing()` (`rs-gl.cpp:254`, frees while context live) but it must be called BEFORE context teardown | **Resolution for P1: call `rs2_gl_shutdown_processing()` (context still current) before destroying the EGL/GLFW context** — a teardown-ORDER requirement, not a missing capability. Optional deeper hardening: guard each GL-object dtor to no-op when `eglGetCurrentContext()==EGL_NO_CONTEXT` (invasive, upstream divergence — defer) | S (in P1) |
