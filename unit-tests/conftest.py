...
if logging.getLoggerClass() is not logging.Logger:
    ...
    for _name, _lgr in list(logging.Logger.manager.loggerDict.items()):
        if (
            isinstance(_lgr, logging.Logger)
            and type(_lgr) is not logging.Logger
            and not _lgr.propagate
        ):
            _lgr.propagate = True
...
# pyrealsense2 Import
# ============================================================================
# pyrealsense2 is built by CMake; repo.find_pyrs_dir() locates the .pyd/.so
...
def pytest_addoption(parser):
    ...
    group.addoption(
        ...
        help="Include only devices matching pattern (e.g., --device D455). "
             "Can be used multiple times or with a space-separated value "
             "(--device 'D455 D435')."
    )
    group.addoption(
        "--exclude-device",
        ...
        help="Exclude devices matching pattern (e.g., --exclude-device D455). "
             "Can be used multiple times or with a space-separated value "
             "(--exclude-device 'D555 D585S')."
    )
    group.addoption(
        "--context",
        action="store",
        default="",
        help="Context for test configuration "
             "(e.g., --context \"nightly weekly\"). Space-separated list."
    )
    group.addoption(
        "--rslog",
        action="store_true",
    ...
    group.addoption(
        ...
        action="store_true",
        default=False,
        help="Only run tests that require a live device "
             "(have at least one device/device_each marker)."
    )
    group.addoption(
        "--not-live",
        action="store_true",
...
def module_device_setup(request):
    ...
    try:
        ...
        module._last_repeat_step = repeat_step
        log.debug("Device enabled and ready")
    except Exception as e:
