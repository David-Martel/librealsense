...
# Module-level state — populated by setup_for_device() before any helper function is called.
device = None
ctx = None
am = None
sensor = None
...
def setup_for_device(test_device):
    """Initialize module globals from a pytest test_device fixture.
...
def test_json_load(config, test_title):
    ...
    check.equal(hdr_config_read.get("hdr-preset"), config.get("hdr-preset"))
...
def perform_manual_hdr_test(hdr_config, test_title, resolution=(640, 480)):
    ...
    for i in range(0, batch_size * 5):
        ...
        check.equal(frame_exposure, expected_exposure,
...
def perform_auto_hdr_test(hdr_config, test_title, resolution=(640, 480)):
    ...
    check.equal(sensor.get_option(rs.option.enable_auto_exposure), 1)
    if sensor.supports(rs.option.auto_exposure_mode):
        check.equal(sensor.get_option(rs.option.auto_exposure_mode), 1)
    ...
    check.is_true(had_any_valid_batch, "No valid batch was captured where all expected exposures were >= 1")
