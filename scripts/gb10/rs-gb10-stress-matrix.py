...
Optional on-screen rendering via OpenCV (default on if DISPLAY set;
--headless to disable).

Run via the GB10 SDK env so the right pyrealsense2 is used:
    realsense-gb10-env python3 rs-gb10-stress-matrix.py \
        [--duration S] [--json PATH] [--headless]
...
DANGER_RE = (
    "Host halt|HC died|not responding|Not enough bandwidth|controller not|"
...
def kernel_danger_since(since_iso):
    ...
    hits = [
        ...
        and "981ae2" not in ln
        and "USBDEVFS_CLEAR_HALT for active endpoint" not in ln
    ]
    return "\n".join(hits[-12:])
...
import numpy as np  # noqa: E402  (deliberate: after the guarded rs import)

# ---- matrix: each entry = list of streams (kind, w, h, fps, fmt) ----
D, C, I = "depth", "color", "infrared"  # noqa: E741  (I = Infrared)


def Z(w, h, fps):
    return (D, w, h, fps, rs.format.z16)


def RGB(w, h, fps):
    return (C, w, h, fps, rs.format.bgr8)


def IR(w, h, fps):
    return (I, w, h, fps, rs.format.y8)


def YUY(w, h, fps):
    return (C, w, h, fps, rs.format.yuyv)
...
MATRIX = [
    ...
    (
        "HEAVY_60fps_848x480_D+C+IR",
        [Z(848, 480, 60), RGB(848, 480, 60), IR(848, 480, 60)],
    ),  # crashed GB10 xHCI 2026-06-02
    ("300fps_848x100_depth", [Z(848, 100, 300)]),
    # --- 720p tier, added 2026-09-10 -----------------------------------------
    # A D435 caps 1280x720 at 30 Hz on depth, color AND infrared -- there is no
    # faster 720p mode on this SKU -- so 30 fps here is the hardware ceiling,
    # not a conservative choice. These are ordered lightest->heaviest like the
    # rest of the matrix, and the 4-stream entry is the heaviest configuration
    # this camera can be asked for: at 30 Hz it moves more bytes per second than
    # HEAVY_60fps_848x480_D+C+IR above, which is the config that killed the GB10
    # xHCI on 2026-06-02. It is deliberately last and behind the same tripwire.
    ("30fps_1280x720_depth", [Z(1280, 720, 30)]),
    ("30fps_1280x720_color", [RGB(1280, 720, 30)]),
    (
        "30fps_1280x720_color_yuyv",
        [YUY(1280, 720, 30)],
    ),  # A/B vs bgr8: host conversion cost
    ("30fps_1280x720_D+C", [Z(1280, 720, 30), RGB(1280, 720, 30)]),
    ("30fps_1280x720_D+IR", [Z(1280, 720, 30), IR(1280, 720, 30)]),
    (
        "30fps_1280x720_D+C+IR",
        [Z(1280, 720, 30), RGB(1280, 720, 30), IR(1280, 720, 30)],
    ),
    (
        "HEAVY_30fps_1280x720_D+C+IR1+IR2",
        [Z(1280, 720, 30), RGB(1280, 720, 30), IR(1280, 720, 30), IR(1280, 720, 30)],
    ),
    (
        "60fps_960x540_D848+C",
        [Z(848, 480, 60), RGB(960, 540, 60)],
    ),  # the >30fps 16:9 option
]
...
def run_entry(name, streams, duration, render):
            ...
            if render and (now - last_draw) >= DRAW_INTERVAL:
                ...
                try:
                    ...
                    for fr in fs:
                        ...
                        imgs.append(a)
                    if imgs:
                        cv2.imshow("rs-stress", np.hstack(imgs))
    ...
    return {
        ...
        "error": err,
        "started": started,
        "aborted_on_fault": aborted_on_fault,
        "ok": ok and started and not aborted_on_fault,
    }
...
def main():
    ...
    ap.add_argument(
        "--duration", type=float, default=12.0, help="seconds per matrix entry"
    )
    ...
    for name, streams in MATRIX:
        ...
        results.append(r)
        s = " ".join(
            f"{x['stream']}={x['fps']}/{x['requested_fps']}fps(g{x['gaps']})"
            for x in r["streams"]
        )
        ...
        if r.get("aborted_on_fault"):
            tag = "ABORT"
            extra = "TRIPWIRE: " + r["aborted_on_fault"].splitlines()[-1][-90:]
        ...
        if (
            r.get("aborted_on_fault")
            or "No device connected" in (r.get("error") or "")
            or kernel_danger_since((datetime.now()).strftime("%Y-%m-%d %H:%M:%S"))
        ):
            aborted = True
            print(
                f"[STOP] sweep halted after '{name}' to protect the xHCI controller.",
                file=sys.stderr,
                flush=True,
            )
            break
