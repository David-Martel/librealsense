...
| `rs-gb10-nonheadless-verify.py` | `just hil-nonheadless [--depth\|--rgbd]` | **Non-headless display + LIVE-video validation on the MAIN display.** Renders the live stream to an on-screen window, `x11grab`s it, and PASS/FAILs on: rendered-on-screen (non-blank) + our-content-on-screen (green-overlay detected) + live-video (two grabs differ) + steady framerate + recorded NVENC `.mp4` + controller-green. **Standardized/reproducible/idempotent** (hil_common preflight + continuous tripwire + timestamped artifact dir + clean teardown; verified 3/3 PASS @ 30 fps). Default single COLOR stream (SAFE); `--depth` colorized depth; `--rgbd` eyes-open. |
