...
- **`ros2-launch-depth-only.sh`** — SAFE single-stream depth-only launch of `realsense2_camera` against the GB10 SDK (848×480×30; `set -eo` not `-u` — ROS2 `setup.bash` uses unbound vars). The node builds + links the SDK; stream-start debugging is tracked (control-path config, not hardware — see ENHANCEMENT-TARGETS / task O3).
